...
* 비동기 호출로 인한 파이어 앤 포겟(Fire-and-Forget) 메시지 유실(Data Loss) 현상을 방지하기 위해 RabbitMQ의 기본 Auto-Ack를 비활성화하고, 비동기 체인이 성공적으로 끝나는 시점에 수동 승인(Manual-Ack) 및 실패 시 Nack(Requeue) 처리를 구현하여 결함 내성이 강한 안전한 논블로킹 파이프라인을 완성했습니다.
